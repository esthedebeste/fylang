...
public:
    ...
    ConstVariable(LLVMValueRef const_load, LLVMValueRef const_ptr) : const_load(const_load), const_ptr(const_ptr) {}
...
public:
    ...
    Type *type;
    BasicLoadVariable(LLVMValueRef variable, Type *type) : variable(variable), type(type) {}
    LLVMValueRef gen_load()
    {
        return LLVMBuildLoad2(curr_builder, type->llvm_type(), variable, "tmpload");
    };
    LLVMValueRef gen_ptr()
    {
        return variable;
